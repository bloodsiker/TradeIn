...
<div class="container">
    ...
    <div>
        ...
        <p>3. Підписуючи даний акт Клієнт цілком усвідомлює та безумовного погоджується віддати у власність Довірителя свій товар описаний в пункті 1 даного акту та в подальшому відмовляється від претензій до {{ $buyBackRequest->user->act_tov }} або до Довірителя.  </p>
        <p>4. Підписанням даного акту Клієнт підтверджує що він повідомлений про можливість придбати зі знижкою новий пристрій в мережі магазинів
            {{ $buyBackRequest->user->act_shop }}. Оціночна вартість старої електронної техніки, яка здається клієнтом,
        ...
        <p>5. Підписанням даного акту клієнт надає згоду на обробку та використання його персональних даних вказаних в даному акті {{ $buyBackRequest->user->act_tov }} та
        ...
        <table>
            <tr>
                <td width="50%">
                    ...
                    <br>
                    <p>{{ $buyBackRequest->user->tov }} в особі керівника магазину розташованого за адресою:</p>
