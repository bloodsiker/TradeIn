                    ...
                    <div class="card-header pd-t-20 d-sm-flex align-items-start justify-content-between bd-b-0 pd-b-0">
                        <div>
                            @if (Auth::user()->isShop())
                                ...
                                @endif
                            @else
                                <h6 class="mg-b-5">Бонусы пользователей</h6>
                            @endif
                        </div>
                            ...
                            <table class="table table-dashboard mg-b-0">
                                ...
                                @foreach($bonuses as $bonus)
                                    <tr>
                                        ...
                                        <td class="tx-medium text-right">{{ $bonus->bonus }} грн</td>
                        ...
                        <div class="table-responsive">
                            <table class="table table-dashboard mg-b-0">
                                ...
                                <tr>
                                    ...
                                    <th class="text-right">Бонус</th>
                                    @if(Auth::user()->isAdmin())
                                        <th class="text-right">Торговая сеть</th>
                                    @endif
                                    <th class="text-right">Магазин</th>
                                    <th class="text-right">Последний визит</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($users as $userBonus)
                                    <tr>
                                        <td class="tx-medium">{{ $userBonus->fullName() }}</td>
                                        <td class="tx-medium text-right">{{ $userBonus->bonus }} грн</td>
                                        <td class="text-right">{{ $userBonus->shop ? $userBonus->shop->name : null }}</td>
                                        @if(Auth::user()->isAdmin())
                                            <td class="text-right">{{ $userBonus->network ? $userBonus->network->name : null }}</td>
                                        @endif
                                        <td class="tx-color-03 text-right">{{ $userBonus->last_online ? \Carbon\Carbon::parse($userBonus->last_online)->format('d.m.Y H:i') : '--.--.----' }}</td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
